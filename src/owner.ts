...
export interface Owner {
  ...
  cwd: string;
  startedAt: number;
  /** The Claude Code process this relay belongs to; when it dies, so does the relay. */
  clientPid?: number;
}
 ...
 */
export function findClientPid(startPid: number = process.pid): number | null {
  let pid = startPid;
  for (let depth = 0; depth < 12 && pid > 1; depth++) {
    ...
    if (!/claude-ping/.test(args) && /(^|\/|\s)claude(\s|$)/.test(args)) return pid;
    pid = parent;
  }
  return null;
}
...
export function liveOwner(): Owner | null {
  const owner = readOwner();
  return owner && isAlive(owner.pid) ? owner : null;
}

export type ClaimResult = { ok: true } | { ok: false; holder: Owner };

export function claimOwnership(owner: Owner): ClaimResult {
 ...
 */
export function releaseOwnership(pid: number = process.pid): void {
  const owner = readOwner();
  if (owner?.pid !== pid) return; // never clear someone else's claim
  try {
    rmSync(OWNER_FILE);
  } catch {
    // Already gone.
 ...
 */
export function clearStaleClaim(): boolean {
  const owner = readOwner();
 ...
 */
export function ownsRelay(sessionId: string, clientPid: number | null = findClientPid()): boolean {
  const owner = liveOwner();
  if (!owner) return false;
  if (owner.clientPid && clientPid) return owner.clientPid === clientPid;
  return owner.sessionId === sessionId;
}

/** What a SessionEnd hook knows about the session that just ended. */
export interface SessionEnd {
  sessionId?: string | undefined;
  clientPid?: number | null;
  /** Claude Code's own word for why: 'clear', 'logout', 'prompt_input_exit', 'other'. */
 ...
 */
export function shouldStopRelay(ending: SessionEnd, owner: Owner | null): boolean {
  if (!owner) return false;
  if (ending.reason === 'clear') return false;
  if (owner.clientPid && ending.clientPid) return owner.clientPid === ending.clientPid;
