...
/**
 * PermissionRequest output. Note this is NOT the PreToolUse shape: that event takes a flat
 * `permissionDecision: allow|deny|ask`, while this one takes a nested `decision.behavior` of
 * allow|deny. Sending the wrong one is accepted silently and then ignored.
 *
 * There is no "ask" here, and none is needed — the dialog is already on screen. Returning an
 * empty object simply leaves it there for the user to answer at the keyboard.
 */
...
export function describeTool(name: string, inp: Record<string, unknown> | undefined): string {
  if (name === 'Bash') return (inp?.['command'] as string) ?? '';
...
export function extractChoices(
...
): { title: string; detail: string; choices: Choice[] } | null {
  if (toolName !== 'AskUserQuestion' || !inp) return null;
  ...
  if (q.multiSelect) return null; // a single tap can't express a multi-select answer
...
export function decide(answer: Answer, toolName: string, question: Question): HookOutput {
...
async function main(): Promise<void> {
  ...
  if (!settings.answerFromPhone || !isConfigured() || !sessionId) return emit({});
  ...
  if (clearStaleClaim()) return emit({});
  if (!ownsRelay(sessionId)) return emit({}); // another session holds the Telegram connection
  ...
  for (let left = holdUntil - Date.now(); left > 0; left = holdUntil - Date.now()) {
    if (settledAtDesktop()) return emit({});
