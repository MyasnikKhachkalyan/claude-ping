...
export function resolvedToolUses(lines: string[]): Set<string> {
  const done = new Set<string>();
  for (const entry of parse(lines)) {
    for (const b of blocks(entry)) {
      if (b?.type === 'tool_result' && b.tool_use_id) done.add(b.tool_use_id);
    }
  }
  return done;
}
...
export function pendingToolUses(
  ...
  for (const entry of parse(lines)) {
    ...
    for (const b of blocks(entry)) {
      if (b?.type !== 'tool_use' || !b.id || b.name !== toolName) continue;
      if (!isDeepStrictEqual(b.input ?? {}, input ?? {})) continue;
      if (!done.has(b.id)) ids.push(b.id);
    }
  }
  return ids;
}
...
export function allResolved(pinned: string[], lines: string[]): boolean {
