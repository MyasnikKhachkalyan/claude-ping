...
import type { FileConfig, Tunables } from './config.js';
...
const out = (obj: Record<string, unknown>): void => console.log(JSON.stringify(obj, null, 2));

function writeConfig(patch: FileConfig): FileConfig {
...
switch (cmd) {
  ...
  case 'off': {
    const value = cmd === 'on';
    const keys: Record<string, keyof FileConfig> = {
    ...
    saveTunable(target, { [key]: value } as Tunables);
  ...
  case 'sessionend': {
    ...
    try {
      const ev = JSON.parse(payload) as { session_id?: string; reason?: string };
  ...
  case 'wait': {
    const which = WAIT_KEYS[arg ?? ''] ? (arg as string) : undefined;
    ...
    if (rawSecs === undefined || rawSecs === '' || !Number.isFinite(secs) || secs < 0) {
      out({
        ok: false,
        error:
          'Usage: setup.js wait [waiting|permission|answer] <seconds> [--global]   (0 = no delay)',
      });
      process.exit(1);
    }
    const capped = which === 'permission' ? Math.min(secs, MAX_PERMISSION_WAIT) : secs;
    const patch: Tunables = which
  ...
  case 'token': {
    ...
    out({
      ok: true,
      bot: (me.result as { username?: string } | undefined)?.username,
      saved: CONFIG_FILE,
    });
    break;
  }
  ...
  case 'detect': {
    ...
    for (const u of (updates.result ?? []) as {
      message?: { chat?: UpdateChat };
      callback_query?: { message?: { chat?: UpdateChat } };
    }[]) {
      const chat = u.message?.chat ?? u.callback_query?.message?.chat;
      if (chat) chats.set(String(chat.id), chat.username ?? chat.first_name ?? '');
    }
